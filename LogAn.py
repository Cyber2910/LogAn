                ...
                if XSS_false_positives_list[element]:
                    for expression in XSS_false_positives_list[element]:
                        if expression.search(Entry.get_req) and xss_list[element].search(query_string):
                            xss_false_detections_list[element].append(print_line(Line_number, line_no, display_options, Entry.ip, Entry.availibility, Entry.user_id, Entry.time, Entry.get_req, Entry.status_code, Entry.size, Entry.referrer, Entry.user_agent, Entry.protacol, Entry.encryption))
                            false_pos_flag = True
                if not false_pos_flag and xss_list[element].search(query_string):
                        xss_detections_list[element].append(print_line(Line_number, line_no, display_options, Entry.ip, Entry.availibility, Entry.user_id, Entry.time, Entry.get_req, Entry.status_code, Entry.size, Entry.referrer, Entry.user_agent, Entry.protacol, Entry.encryption))
                ...
                if SQL_false_positives_list[element]:
                    for expression in SQL_false_positives_list[element]:
                        if expression.search(Entry.get_req) and sql_list[element].search(query_string):
                            sql_false_detections_list[element].append(print_line(Line_number, line_no, display_options, Entry.ip, Entry.availibility, Entry.user_id, Entry.time, Entry.get_req, Entry.status_code, Entry.size, Entry.referrer, Entry.user_agent, Entry.protacol, Entry.encryption))
                            false_pos_flag = True
                if not false_pos_flag and sql_list[element].search(query_string):
                    sql_detections_list[element].append(print_line(Line_number, line_no, display_options,Entry.ip, Entry.availibility, Entry.user_id, Entry.time, Entry.get_req, Entry.status_code, Entry.size, Entry.referrer, Entry.user_agent, Entry.protacol, Entry.encryption))
...
display_options = initiate_display_options(Client_IP,Information_avalibility,User_ID,Time,Request_line,Status_code,Size,Ref_display,User_Agent,Protacol,Encryption)
...
pattern = new_re.compile(r'^(\S*|\S*\,\s\S+) (\S+) (\S+(?:\s+)?) (\[.+?\]) (\"(GET|HEAD|POST|PROPFIND|-|OPTIONS|PUT|PATCH|quit).*?\") (\d{3}) (\S+) ((\".*?\")) (\".*?\") (\S+) (\S+)')
                ...
                try:
                    for j, line in enumerate(infile,1):
                        ...
                        if Filters(status_list, Entry.status_code, date_regex, Entry.time, time_capture_regex, Time_range, Entry.user_agent, bot_reg, Entry.ip, IP_list, Filter_type, Entry.referrer, referrer_list):
                            if Print_all:
                                print_all_file.write(print_line(Line_number, j, display_options, Entry.ip, Entry.availibility, Entry.user_id, Entry.time, Entry.get_req, Entry.status_code, Entry.size, Entry.referrer, Entry.user_agent, Entry.protacol, Entry.encryption) + '\n')

                            xss_sql_attack_detect(Entry, j, display_options, XSS, SQL)

                            if Email_leaks:
                                if emails_detect(1, Entry, j, emails_detect_regex):
                                    emails_get_list.append(print_line(Line_number, j, display_options, Entry.ip, Entry.availibility, Entry.user_id, Entry.time, Entry.get_req, Entry.status_code, Entry.size, Entry.referrer, Entry.user_agent, Entry.protacol, Entry.encryption))
                                if emails_detect(2, Entry, j, emails_detect_regex):
                                    emails_referrer_list.append(print_line(Line_number, j, display_options, Entry.ip, Entry.availibility, Entry.user_id, Entry.time, Entry.get_req, Entry.status_code, Entry.size, Entry.referrer, Entry.user_agent, Entry.protacol, Entry.encryption))
                        
                            if File_inlcusion:
                                if OS_files_keyword_search and path_checker(Entry.get_req, 1):
                                    os_path_list.append(print_line(Line_number, j, display_options, Entry.ip, Entry.availibility, Entry.user_id, Entry.time, Entry.get_req, Entry.status_code, Entry.size, Entry.referrer, Entry.user_agent, Entry.protacol, Entry.encryption))
                                if Other_suspicious_keywords_search and path_checker(Entry.get_req, 2):
                                    restricted_files_list.append(print_line(Line_number, j, display_options, Entry.ip, Entry.availibility, Entry.user_id, Entry.time, Entry.get_req, Entry.status_code, Entry.size, Entry.referrer, Entry.user_agent, Entry.protacol, Entry.encryption))
                                if Slash_search_in_query and path_query(Entry.get_req, j, path_query_regex):
                                    path_list.append(print_line(Line_number, j, display_options, Entry.ip, Entry.availibility, Entry.user_id, Entry.time, Entry.get_req, Entry.status_code, Entry.size, Entry.referrer, Entry.user_agent, Entry.protacol, Entry.encryption))
                                if Apply_regex_search and path_traversal_detect(Entry.get_req, path_traversal_regex):
                                    file_inlcusion_regex_list.append(print_line(Line_number, j, display_options, Entry.ip, Entry.availibility, Entry.user_id, Entry.time, Entry.get_req, Entry.status_code, Entry.size, Entry.referrer, Entry.user_agent, Entry.protacol, Entry.encryption))
                                if URL_and_file_extension_query and URL_and_file_extension_func(Entry.get_req):
                                    url_and_extension_list.append(print_line(Line_number, j, display_options, Entry.ip, Entry.availibility, Entry.user_id, Entry.time, Entry.get_req, Entry.status_code, Entry.size, Entry.referrer, Entry.user_agent, Entry.protacol, Entry.encryption))
